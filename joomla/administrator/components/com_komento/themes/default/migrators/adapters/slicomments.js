Komento.ready(function($){
	...
	$('[data-migrate-slicomments]').on('click', function() {
		...
		{
			if (!postIds) {
				$('[data-migrate-slicomments]').html('<i class="fa fa-check"></i> <?php echo JText::_('No post to be migrated for the selected category');?>');
				return;
			}
		...
		$.each(postIds, function(index, value) {
		  ...
		  .done(function(results)
		  {
		  	if (index === total - 1) {
		  		...
		  		$('[data-migrate-slicomments]').removeAttr('disabled');
		  		$('[data-migrate-slicomments]').html('<i class="fa fa-check"></i> <?php echo JText::_('COM_KOMENTO_COMPLETED', true);?>');
		  	}
		  });
		});
	}
