...
class JFormFieldModal_Page extends JFormField
{
	...
	{
		// Load language
		JFactory::getLanguage()->load('com_jwpagefactory', JPATH_ADMINISTRATOR);
		...
		$html	= array();
		$link	= 'index.php?option=com_jwpagefactory&amp;view=pages&amp;layout=modal&amp;tmpl=component&amp;function=jSelectPage_' . $this->id;
		...
		if ((int) $this->value > 0)
		{
			...
			$query = $db->getQuery(true)
				...
				->from($db->quoteName('#__jwpagefactory'))
		...
		if (empty($title))
		{
			$title = JText::_('COM_JWPAGEFACTORY_SELECT_AN_PAGE');
		}
		...
		$html[] = '<a href="#modalPage' . $this->id . '" class="btn hasTooltip" role="button"  data-toggle="modal" title="'
			. JHtml::tooltipText('COM_JWPAGEFACTORY_CHANGE_PAGE') . '">'
