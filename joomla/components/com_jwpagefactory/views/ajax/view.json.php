...
$authorised = $user->authorise('core.edit', 'com_jwpagefactory') || ($user->authorise('core.edit.own', 'com_jwpagefactory') && ($this->item->created_by == $user->id));
if ($authorised !== true) {
...
if ($action === 'addon') {
	...
	if (class_exists($class_name)) {
		...
		$addon_output = JwpfAddonParser::jwDoAddon(JwpfAddonParser::generateShortcode($addon, 0, 0));
	}
...
if ($action === 'get-page-data') {
	...
	if (JFile::exists($page_path)) {
		...
		if (is_array(json_decode($content))) {
			...
			$content = JwPageFactoryAddonHelper::__($content, true);
...
if ($action === 'setting_value') {
	...
	$addon_id = $_POST['id'];
	JwPageFactoryBase::loadSingleAddon($addon_name);
	$form_fields = JwAddonsConfig::$addons;

	$first_attr = current($form_fields[$addon_name]['attr']);
	$options = JwPageFactoryBase::addonOptions();
