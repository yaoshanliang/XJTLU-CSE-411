...
if ($btn_style == 'link') {
	...
	$css .= $addon_id . ' #' . $id . '.jwpf-btn-link:hover,';
	$css .= $addon_id . ' #' . $id . '.jwpf-btn-link:focus {';
	$css .= $link_hover_style;
	$css .= '}';
}
if ($style) {
	$css .= $addon_id . ' #' . $id . '.jwpf-btn-' . $btn_style . '{' . $style . '}';
}

if ($btn_style == 'custom') {
	if ($custom_style) {
		$css .= $addon_id . ' #' . $id . '.jwpf-btn-custom {' . $custom_style . '}';
	}

	if ($hover_style) {
		$css .= $addon_id . ' #' . $id . '.jwpf-btn-custom:hover {' . $hover_style . '}';
	}

	// Responsive Tablet
	if (!empty($custom_style_sm)) {
		$css .= "@media (min-width: 768px) and (max-width: 991px) {";
		$css .= $addon_id . ' #' . $id . ".jwpf-btn-custom {\n" . $custom_style_sm . "}\n";
		$css .= "}";
	}
