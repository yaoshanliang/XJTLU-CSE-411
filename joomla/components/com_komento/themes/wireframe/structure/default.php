			...
			<?php if ($this->config->get('show_sort_buttons') || $this->config->get('enable_rss') || $showSubscribe) { ?>
				<div class="kt-dashboard-tab t-lg-mb--md">
					...
					<?php if ($this->config->get('show_sort_buttons')) { ?>
						<div class="kt-dashboard-tab__item <?php echo $activeSort == 'oldest' ? 'is-active' : '';?>">
							<a href="javascript:void(0);" class="kt-dashboard-tab__link <?php echo $activeSort == 'oldest' ? 'is-active' : ''; ?>" data-kt-sorting data-type="oldest">
								<?php echo JText::_('COM_KOMENTO_SORT_OLDEST_FIRST');?>
							</a>
						</div>

						<div class="kt-dashboard-tab__item <?php echo $activeSort == 'latest' ? 'is-active' : '';?>">
							<a href="javascript:void(0);" class="kt-dashboard-tab__link <?php echo $activeSort == 'latest' ? 'is-active' : ''; ?>" data-kt-sorting data-type="latest">
								<?php echo JText::_('COM_KOMENTO_SORT_NEWEST_FIRST');?>
							</a>
						</div>

						<div class="kt-dashboard-tab__item <?php echo $activeSort == 'popular' ? 'is-active' : '';?>">
							<a href="javascript:void(0);" class="kt-dashboard-tab__link <?php echo $activeSort == 'popular' ? 'is-active' : ''; ?>" data-kt-sorting data-type="popular">
								<?php echo JText::_('COM_KT_SORT_MOST_LIKES');?>
							</a>
						</div>
					<?php } ?>

					<?php if ($this->config->get('enable_rss') || $showSubscribe) { ?>
						<div class="kt-dashboard-tab__item kt-dashboard-tab__item--right kt-dashboard-tab__item--no-divider t-lg-pr--no">
							...
							<?php if ($showSubscribe) { ?>
								<?php if (is_null($subscription)) { ?>
									<a href="javascript:void(0);" class="kt-email-btn t-lg-ml--lg" data-kt-subscribe data-kt-provide="tooltip" data-title="<?php echo JText::_('COM_KOMENTO_SUBSCRIBE_EMAIL'); ?>">
										<i class="fa fa-envelope"></i>
									</a>
								<?php } else { ?>
									<a href="javascript:void(0);" class="kt-email-btn t-lg-ml--lg" data-kt-unsubscribe data-kt-provide="tooltip" data-title="<?php echo JText::_('COM_KOMENTO_FORM_UNSUBSCRIBE'); ?>">
										<i class="fa fa-envelope"></i>
									</a>
								<?php } ?>
							<?php } ?>
