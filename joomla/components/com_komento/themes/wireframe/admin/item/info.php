...
if( $this->config->get( 'enable_info' ) && $row->modified_by ) { ?>
<span class="commentInfo kmt-info">
	<?php $modified = $row->modified;
	if( $this->config->get( 'enable_lapsed_time' ) ) {
		$modified = KT::getHelper( 'date' )->getLapsedTime( $row->modified );
	} ?>
	<?php echo JText::sprintf( 'COM_KOMENTO_COMMENT_EDITTED_BY', $modified, KT::getProfile( $row->modified_by )->getName() ); ?>
	...
	<span class="hidden" itemprop="editor" itemscope itemtype="http://schema.org/Person">
		<span itemprop="name"><?php echo KT::getProfile( $row->modified_by )->getName(); ?></span>
	</span>
