...
<h3 class="kmt-author" itemprop="creator" itemscope itemtype="http://schema.org/Person">
	<?php if( $row->author->guest ) {
		if( !empty( $row->url ) && $this->config->get( 'enable_guest_link' ) ) { ?>
			<a href="<?php echo $this->escape( $row->url ); ?>"<?php echo $nofollow; ?><?php if( $this->config->get( 'enable_schema' ) ) echo ' itemprop="url"'; ?>>
		<?php }
	} else { ?>
		<a href="<?php echo $row->author->getProfileLink( $this->escape( $row->email ) ); ?>"<?php if( $this->config->get( 'enable_schema' ) ) echo ' itemprop="url"'; ?>>
	<?php } ?>
